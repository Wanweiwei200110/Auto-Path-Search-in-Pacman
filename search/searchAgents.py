...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST,
            ...
            psn = (nextx, nexty)
            if not self.walls[nextx][nexty]:
                nextState = [psn, state[1]]
                if psn in state[1]:
                    new_state = list(state[1])
                    new_state.remove(psn)
                    nextState = [(psn), tuple(new_state)]
...
def cornersHeuristic(state, problem):
    ...
    for corner in state[1]:
        dist.append((util.manhattanDistance(state[0], corner), corner))
    dist.sort()
    h = dist[0][0]
    while len(dist) > 1:
        for i in range(1, len(dist)):
            dist2.append(
                (util.manhattanDistance(dist[0][1], dist[i][1]), dist[i][1]))
        dist2.sort()
        h += dist2[0][0]
        dist = dist2[:]
        dist2 = []
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    psns = foodGrid.asList()
    if len(psns) == 0:
        return 0
    if len(psns) == 1:
        return util.manhattanDistance(psns[0], position)
    candidates = []
    dst_to_psn = []
    for i in range(len(psns)):
        dst_to_psn.append(util.manhattanDistance(psns[i], position))
        for j in range(i+1, len(psns)):
            candidates.append((util.manhattanDistance(psns[i], psns[j]), psns[i], psns[j]))
    candidates.sort()
    mst = [candidates[0][1], candidates[0][2]]
    h = candidates[0][0]
    candidates.pop(0)
    while len(mst) < len(psns):
        for candidate in candidates:
            if candidate[1] in mst and candidate[2] not in mst:
                h += candidate[0]
                mst.append(candidate[2])
                break
            if candidate[2] in mst and candidate[1] not in mst:
                h += candidate[0]
                mst.append(candidate[1])
                break
    return h + min(dst_to_psn)
